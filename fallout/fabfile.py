...
dev_servers = {
    '197': {
        'pythonpath': '--redacted--/pybin/fallout',
        'uwsgi': 'fallout_alpha.xml',
    },
    '198': {
        'pythonpath': '--redacted--/pybin/fallout',
        'uwsgi': 'fallout_beta.xml',
    },
}
prod_server = {
    'pythonpath': '--redacted--/pybin/fallout',
    'uwsgi': 'fallout_production.xml',
}
    ...
    if _authenticate(conn):
        with conn.cd(conn.pysetting.get('pythonpath')):
            if branch:
                ...
                conn.run('git pull')
            conn.run('../venv/fallout/bin/pip install -Ur requirements.txt')
            conn.run('../venv/fallout/bin/python manage.py migrate')
            conn.run('../venv/fallout/bin/python manage.py collectstatic --noinput')
...
def celery(connection, server, action='restart'):
    ...
    if server in dev_servers.keys():
        ...
        celery_conn.pysetting = {
            'celeryservicerun':  'sudo /sbin/service celery-www{}-fallout {}'.format(server, action)
        }
    elif server == 'prod':
        celery_conn = Connection('irep_dmz@celery-dmzst')
        celery_conn.pysetting = {
            'celeryservicerun':  'sudo /sbin/service celery-fallout {}'.format(action)
        }
    else:
        raise Exception('Unknown Server: {}'.format(server))
