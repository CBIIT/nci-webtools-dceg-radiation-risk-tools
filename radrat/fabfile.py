...
dev_servers = {
    '197': {
        'pythonpath': '--redacted--/pybin/radrat',
        'uwsgi': 'radrat_alpha.xml',
    },
    '198': {
        'pythonpath': '--redacted--/pybin/radrat',
        'uwsgi': 'radrat_beta.xml',
    },
}
prod_server = {
    'pythonpath': '--redacted--/pybin/radrat',
    'uwsgi': 'radrat_production.xml',
}
    ...
    if _authenticate(conn):
        with conn.cd(conn.pysetting.get('pythonpath')):
            if branch:
                ...
                conn.run('git pull')
            conn.run('../venv/radrat/bin/pip install -Ur requirements.txt')
            conn.run('../venv/radrat/bin/python manage.py migrate')
            conn.run('../venv/radrat/bin/python manage.py collectstatic --noinput')
...
def celery(connection, server, action='restart'):
    ...
    if server in dev_servers.keys():
        ...
        celery_conn.pysetting = {
            'celeryservicerun':  'sudo /sbin/service celery-www{}-radrat {}'.format(server, action)
        }
    elif server == 'prod':
        celery_conn = Connection('irep_dmz@celery-dmzst')
        celery_conn.pysetting = {
            'celeryservicerun':  'sudo /sbin/service celery-radrat {}'.format(action)
        }
    else:
        raise Exception('Unknown Server: {}'.format(server))
